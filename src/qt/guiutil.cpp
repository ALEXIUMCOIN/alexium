...
{
    // Convert alexium:// to alexium:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("alexium://"))
    {
        uri.replace(0, 12, "alexium:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Alexium.lnk";
}
...
{
    return GetAutostartDir() / "alexium.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Alexium-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  alexium-qt [" + tr("command-line options") + "]                     " + "\n";
